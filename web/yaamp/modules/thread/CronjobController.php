...
//		BackendRunCoinActions();
		...
		debuglog("---Running Coin Payments---");

		$mining->last_payout = time();
		$mining->save();

		$btc_mining = getdbosql('db_mining');
		if($btc_mining->btc_last_payout + YAAMP_BTC_PAYMENTS_FREQ > time()) return;

		debuglog("---Running BTC Payments---");

		$btc_mining->btc_last_payout = time();
		$btc_mining->save();
