...
static void equi_hash(crypto_generichash_blake2b_state *S, const uint32_t g, uint8_t *hash, const size_t hashLen) {
...
bool verifyEH(const char *hdr, const char *soln) {
  ...
  for (int j = 0; j < solnr; j++) {
  	...
  	equi_hash(&state, i / indicesPerHashOutput, tmpHash, hashOutput);
